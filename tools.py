...
FEROX_LOCK = threading.Lock() # Lock to prevent parallel feroxbuster runs
...
def run_feroxbuster_tool(url: Union[str, List[str]], extensions: str = "php,html,js,txt", verbose: bool = False) -> str:
    ...
    with FEROX_LOCK:
        ...
        if all_findings:
            ...
            return f"Feroxbuster complete for {len(new_targets)} targets. Found {len(all_findings)} interesting items."
